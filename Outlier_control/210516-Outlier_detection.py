...
##########데이터 로드 (최근 본 이미지 데이터) 임시로 픽사베이 이미지로 설정해둠
...
for i in range(len(recent_data)):
    ...
    print(i+1,"번째 이미지는")
    max_cate = first_max(recent_data_list)
    sec_max_cate = second_max(recent_data_list)
    trd_max_cate = third_max(recent_data_list)
    print("--------------------------------------")
...
print(Counter(first_list))
print(Counter(second_list))
print(Counter(third_list))
print(Counter(first_list).most_common(1)[0][0])
print(Counter(second_list).most_common(1)[0][0])
print(Counter(third_list).most_common(1)[0][0])
...
"""
start = time.time()
print(train_df[first_most_com].sort_values())
print(train_df[first_most_com].min()) #2
print(train_df[first_most_com].max()) #91
print(train_df[first_most_com].median()) #6.0

first_quartile_0 = train_df[first_most_com].quantile(0) #percentile (같은 데이터 수로 100 등분), decile (같은 데이터 수로 10 등분), quartile (같은 데이터 수로 4 등분) #최소값
first_quartile_1 = train_df[first_most_com].quantile(0.25) #1 사분위수 (25% 지점수)
first_quartile_2 = train_df[first_most_com].quantile(0.5) #2 사분위수 (50% 지점수, 중앙값 (median))
first_quartile_3 = train_df[first_most_com].quantile(0.75) #3 사분위수 (70% 지점수)
first_quartile_4 = train_df[first_most_com].quantile(1) #최대값

first_IQR = first_quartile_3 - first_quartile_1
print(first_quartile_1 - first_IQR) #-5.25
print(first_quartile_3 + first_IQR) #16.75

search_df = train_df[(train_df[first_most_com] < (first_quartile_1 - first_IQR)) | (train_df[first_most_com] > (first_quartile_3 +  first_IQR))]
print(search_df)
...
def find_Outlier(input_most_common):
    ...
    return search_df
findyourway = find_Outlier(first_most_com)
